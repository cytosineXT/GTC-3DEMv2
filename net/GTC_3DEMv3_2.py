...
# 3. (新增)实时互易性约束 (On-the-fly Reciprocity Constraint): 在每个训练步中，通过一次额外的网络推理来强制网络输出满足互易定理，
#    这等价于一次无限的数据增强，能极大提升模型的物理一致性和泛化能力。
...
class MeshCodec(Module):
    def __init__(
        ...
        # (新增) 初始化互易性损失的权重
        self.lambda_reciprocity = kwargs.get('lambda_reciprocity', 0.1)
    ...
    def encode(self, *, vertices, faces, face_edges, in_em):
        ...
        in_freq = original_freqs.float().unsqueeze(1).unsqueeze(1).to(device) # 使用恢复后的原始频率
